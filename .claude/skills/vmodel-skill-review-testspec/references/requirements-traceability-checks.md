...
When the TestSpec is non-leaf (branch OR root), walk the layer's Requirements document and verify each load-bearing requirement has at least one corresponding case. **Position C: layer Requirements is a verification target at both branch and root** (per TARGET_ARCHITECTURE §5.3 "Verification targets per scope"). At root scope, additionally walk the root product (PB / needs / PD) and verify each load-bearing element (PB outcome / needs entry / PD section) has at least one corresponding case at root level. Conditional gating: branch AND root scope (root-product checks: root only).
