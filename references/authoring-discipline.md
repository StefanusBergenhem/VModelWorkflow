...
source_of_truth: this file ships as a framework default. Init copies it to the project's `.vmodel/references/authoring-discipline.md`; skills resolve the project copy via `.vmodel/config.yaml` `paths.references`. Projects may override.
---
...
**Allowed.**
- Inline `[DEFER-DD: ...]` / `[DEFER-ADR: ...]` / `[NEEDS-TEST: ...]` markers — single mechanism for naming product-spec gaps. The auto-generated aggregate at `.vmodel/defer-index.md` collates them.
- Front-matter metadata (status, version, derivation links, recovery_status).
- *Glossary*.

**Killed.** The previous `## Open follow-ups` section is no longer authored on any artifact. DEFER markers replace it.
...
This file ships as a framework default at `references/authoring-discipline.md`. The init skill (`vmodel-init`) copies it into the project at `.vmodel/references/authoring-discipline.md`. Skills resolve the project copy via `.vmodel/config.yaml` `paths.references` and never hardcode framework paths. Projects may override the file in place; on framework upgrade, the migrate flow surfaces drift.

Affected skills (10): `vmodel-skill-author-{requirements, architecture, detailed-design, testspec, adr}` and `vmodel-skill-review-{requirements, architecture, detailed-design, testspec, adr}`. The previous per-skill bundled copies and `scripts/sync-authoring-discipline.sh` are retired.
