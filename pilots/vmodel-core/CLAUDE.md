...
- **Issue 25 (six-vs-seven schemas) resolved 2026-05-12** — Option 1 taken; `architecture-interface-detail` promoted to a first-class artifact type. Requirements Glossary v2 names seven types; `DD-embedded-resources` v2 carries the seven-element `ArtifactType` enum; all eight pilot interface-detail files now schema-valid; new QB at `schemas/artifacts/quality-bar/architecture-interface-detail.quality-bar.json`. `DD-validation-engine` is unblocked.
