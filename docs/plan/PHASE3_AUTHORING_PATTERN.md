...
| # | Task | Notes |
|---|---|---|
| 12 | Extract Quality Bar JSON — test-spec (5/5) | Four of five complete (commits c3b6800, 1ae007e, 7a4920a, 81b20f7 via parallel Sonnet subagents). TestSpec remaining — held for last because of its "Per-layer weight" group, which is the only group across all six artifacts where items legitimately split by scope level (leaf / branch / root). Will be the second real-world `applies_at` test and covers the leaf+branch layers that Architecture didn't exercise. Completion activates TRV-QB-001 (cascade rule — currently `deferred_until: phase_3_task_3_and_12`). |
| 13 | Minimal-example fixtures per artifact | Depends on all schemas. Six per-artifact schemas + traceability schemas + QB container all in place. Independent of #12; can run in parallel. |
...
Four of five QB extractions landed this session via parallel Sonnet subagents. Only TestSpec remains on task #12, plus task #13 fixtures.

**Task #12 5/5 — TestSpec QB.** Held for last because TestSpec's "Per-layer weight" group is the only group across all six artifacts where items legitimately split by scope level. This will be the second real-world `applies_at` test (Architecture was the first, root-only; TestSpec exercises leaf + branch + root). Expect items like "At leaf: cases carry title + type + verifies + inputs + expected; steps usually omitted" → `applies_at: ["leaf"]`. Same extraction pattern as the four landed this session; single-agent or main-context extraction is fine given the shape is proven. Completion activates TRV-QB-001 (cascade rule — currently `deferred_until: phase_3_task_3_and_12`).

Parallel: **Task #13 — Minimal-example fixtures per artifact.** Six minimal valid Markdown instances (one per artifact type) that round-trip through the corresponding schema's front-matter validator. Golden set for downstream tool testing (parser, scaffolder, validator); catches any subtle schema-vs-HTML mismatches before Phase 5 skills start consuming the schemas. Independent of #12.

**Phase 3 closeout signal.** After #12 5/5 and #13, Phase 3 is complete. A final commit should flip TRV-QB-001 from `deferred_until` to `status: active` (schemas/traceability/validation-rules.catalog.json) and update BACKLOG §3.3 task-list to reflect landed state (the per-artifact schema entries still show `.schema.yaml` filenames — broader drift noted in the handoff doc §1 commit history).
