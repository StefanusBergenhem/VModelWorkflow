...
**Phase 3 work to date (14 commits):**
...
| # | Task | Notes |
|---|---|---|
| 12 | Extract Quality Bar JSON for remaining five artifacts | ADR exemplar landed under #3. Remaining: product-brief, requirements, architecture, detailed-design, test-spec. Mechanical extraction against the now-locked container shape; one commit per artifact per Phase 3 cadence. Unblocks TRV-QB-001 cascade on completion. |
...
All schema-shape work is done, including the Quality Bar container. Two Phase-3 tracks remain — per-artifact QB extraction (task #12) and fixtures (task #13). Both are unblocked.

**Task #12 — extract QB JSON for the remaining five artifacts.** Mechanical per-artifact extraction against the now-locked container (`quality-bar.schema.json`). One commit per artifact matches Phase 3 cadence. Suggested order by ramp-up: product-brief → requirements → detailed-design → architecture → test-spec (TestSpec last: it uses `applies_at` in the "Per-layer weight" group — the first real test of that mechanism in the wild). Completion activates TRV-QB-001 (Quality Bar cascade — currently `deferred_until: phase_3_task_3_and_12` in `validation-rules.catalog.json`).

Alternative: **Task #13 — Minimal-example fixtures per artifact.** Write one minimal valid instance per artifact type (six Markdown files) that round-trips through the corresponding schema's front-matter validator. Establishes a golden set for downstream tool testing (parser, scaffolder, validator) and catches any subtle schema-vs-HTML mismatches before Phase 5 skills start consuming the schemas. Independent of #12; can run in parallel.
