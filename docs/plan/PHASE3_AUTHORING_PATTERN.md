...
**Phase 3 work to date (12 commits):**
...
| 11 | traceability link-types + validation rules | Parallel-able with per-artifact work; good short-session candidate. All six per-artifact schemas now in place. |
| 12 | Extract Quality Bar YAML per artifact | Depends on #3. |
| 13 | Minimal-example fixtures per artifact | Depends on all schemas. All six per-artifact schemas now in place. |
...
All six per-artifact schemas now in place. Remaining Phase 3 work splits cleanly between traceability and Quality Bar infrastructure.

**Task #11 — traceability link-types + validation rules.** TARGET §7 is well-specified (canonical link types table, reverse-derived relationships, reference integrity / completeness / cycles / retrofit validation rules). The schema side is bounded: a link-types catalog (`derived_from`, `allocates`, `verifies`, `governing_adrs`, `supersedes` / `superseded_by`, `scope_tags`, `affected_scopes`) with source / target artifact-type constraints, and a validation-rules module encoding the cross-artifact completeness rules (every req allocated, every child allocates a req, every TC has non-empty `verifies`, every leaf has DD+TestSpec, every non-leaf has Reqs+Arch+TestSpec, no circular `derived_from` / `supersedes`). Parallel-able with any other Phase 3 work; good short-session candidate now that all six per-artifact schemas have committed `governing_adrs` / `derived_from` / `verifies` / `supersedes` patterns that the link-types catalog can reference for target validation.

Alternative 1: **Task #3 — Quality Bar YAML container shape.** Its own session. Blocks task #12 (per-artifact QB extraction). HTML sections (`docs/guide/artifacts/*.html` §5) are authoritative for content; task is purely the container shape. Natural next step now that all six per-artifact schemas have landed — the QB extraction can reference the stable ID patterns and field names from the schemas.

Alternative 2: **Task #13 — Minimal-example fixtures per artifact.** Write one minimal valid instance per artifact type (six Markdown files) that round-trips through the corresponding schema's front-matter validator. Establishes a golden set for downstream tool testing (parser, scaffolder, validator) and catches any subtle schema-vs-HTML mismatches before Phase 5 skills start consuming the schemas. Can run in parallel with #11 or #3.
